    ...
    <div className="bg-zinc-900 rounded-3xl overflow-hidden card transition-all duration-300 ease-in-out hover:translate-y-[-12px]">
      ...
      <div className="p-6">
        ...
        <div className="mt-6 flex items-center justify-end">
          <button 
            onClick={handleLike}
            className={`w-full py-4 rounded-2xl ${isLiked ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-violet-600 hover:bg-violet-500'} text-white font-medium flex items-center justify-center gap-2`}
          >
            {isLiked ? 'Взаимно!' : 'Нравится'}
            <FaHeart className={`${isLiked ? 'text-white' : 'text-gray-100'}`} />
          </button>
        </div>
      </div>
    </div>
